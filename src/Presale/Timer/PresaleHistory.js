...
const PresaleHistory = () => {
  ...
  const combineRealAndSimulatedData = (realHistory, simulatedHistory) => {
    ...
    console.log("🎭 Simulated history:", simulatedHistory);
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        try {
          console.log("🎭 [PresaleHistory] Fetching simulated data from backend:", `${API_URL}/api/presale/history`);
          const res = await axios.get(`${API_URL}/api/presale/history`);
          console.log("🎭 [PresaleHistory] Backend simulated response:", res.data);
          simulatedHistory = Array.isArray(res.data) ? res.data : [];
        } catch (backendErr) {
          console.warn("⚠️ [PresaleHistory] Backend unavailable for simulated data:", backendErr.message);
          simulatedHistory = [];
        }
        ...
        if (realHistory.length > 0) {
          ...
          console.log("🎭 [PresaleHistory] Only simulated data available");
          ...
          setError("⚠️ Showing simulated data only - no real blockchain sales detected");
        } else {
          // No data at all
